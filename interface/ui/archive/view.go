...
var (
	...
	titleStyle        = lipgloss.NewStyle().Reverse(true).PaddingLeft(1).Italic(true).Width(18).Foreground(lipgloss.Color(constants.ArchiveBgColor))
	...
	previewStyle      = lipgloss.NewStyle().Margin(1, 1, 0, 4).Padding(1).Foreground(lipgloss.Color("#FAFAFA")).Background(lipgloss.Color(constants.ArchiveBgColor)).Border(lipgloss.NormalBorder(), false, false, false, true)
)
...
func (m model) View() string {
	...
	preview := m.previewView(width, m.height+adjustedHeight)
...
func (m model) pageTitleView(width int) string {
	return titleAreaStyle.Width(width).Render(titleStyle.Render("# Archived Items"))
}

func (m model) listView(width, height int) string {
	m.list.SetHeight(height)
	m.list.Styles.NoItems = noItemStyle.Width(41)
